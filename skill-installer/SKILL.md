---
name: skill-install
description: Install agent skills from Git repositories into the local ~/skills directory, organized by maintainer. Use when the user wants to install, download, or fetch skills from GitHub or any Git repository, or when a skill is referenced but not yet available locally. Automatically clones repos and organizes skills in the ~/skills/<maintainer>/ folder structure. Works with a specific repo URL or defaults to the official Anthropic skills repository (https://github.com/anthropics/skills.git). Trigger this skill whenever the user mentions installing skills, downloading skills, cloning skill repos, setting up skills, or bootstrapping a skills environment, even if they don't explicitly say "install skills".
---

# Skill Install

Install agent skills from Git repositories into a structured `~/skills/<maintainer>/<repo-name>` directory. This skill enables bootstrapping a complete skills environment on a fresh machine with only `git`, `python3`, `nodejs`, and `curl` available.
...
If no repository URL is provided, the skill defaults to installing Anthropic's official skills:

> "Install the default skills"
> "Set up my skills environment"
> "Bootstrap skills"

This clones `https://github.com/anthropics/skills.git` into `~/skills/anthropics/skills/`.
...
**Important**: Always use `--ff-only` when pulling to avoid merge conflicts. If the pull fails due to divergence, inform the user and suggest removing the existing directory first.
...
- **Pull conflict (divergent branches)**: Inform the user and suggest: `rm -rf ~/skills/<maintainer>/<repo-name> && git clone <url> ~/skills/<maintainer>/<repo-name>`
...
To update all installed skill repos at once:
...
for maintainer_dir in "$HOME/skills"/*/; do
  ...
  for repo_dir in "$maintainer_dir"*/; do
    if [ -d "$repo_dir/.git" ]; then
      ...
      echo "⬆️  Updating $maintainer/$repo_name..."
      cd "$repo_dir" && git pull --ff-only 2>&1 || echo "⚠️  Failed to update $maintainer/$repo_name"
    fi
  done
done
echo ""
...
→ Runs git pull --ff-only in each repo under ~/skills/<maintainer>/<repo>/
→ Reports updated skills per maintainer
```

**Example 5: List installed skills**
```
