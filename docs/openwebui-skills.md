...
Manifest injection - Only the skill's name and description are added to the system prompt.
On-demand loading - The model receives a view_skill builtin tool. When it determines it needs a skill's full instructions, it calls view_skill(skill_name) to load them.
...
Attached skills still require user access
